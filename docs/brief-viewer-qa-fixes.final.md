...
`web/viewer-maplibre.js` の `CONTROL_DEFS`、`riderScale` スライダーの `max` を 80→500 に変更済 (表示 1.0×〜50.0×、既定3.6×据え置き)。main が実装し未コミット。worker はこれを取り込みバッチのコミットに含める。下記 item 3 はこの「巨大ライダー可能化」が露出させた副作用。
