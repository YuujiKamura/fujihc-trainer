...
- `web/viewer-map3d.js` の 2258 行目あたり、`CONTROL_DEFS` の `labelHeight` エントリ。初期値 `value:4`、範囲 1〜20m、ステップ 1。スライダー操作で `mapRenderer.setLabelHeight(raw)` を呼ぶ。
...
3. 決めた値で `LABEL_BASE_HEIGHT_M`（labels3d.js）と `labelHeight` CONTROL_DEF の `value`（viewer-map3d.js）を更新。他にデフォルトを焼き込んでいる箇所がないか grep で確認し、あれば揃える。
...
- `web/lib/terrain3d.js` は配布元配慮で無改造。1 文字も触るな。`labels3d.js` / `viewer-map3d.js` / `labels3d.test.js` は変更してよい。
