"""Arabas 2015 を強上昇流 (= rhod_w_max=2.0) で再走、 cumulus 寄り設定、 4 倍 domain (b113)。
...
注意: 6km domain では雲が大きく育ち、 雨粒の最大半径が 6mm を超えて Gunn-Kinzer
落下速度補間範囲外に出ると PySDM が ValueError で正常停止する。 b113 の実走は
sim_time 5400s 目標のうち約 70% (= 65 step) で停止、 ql max 9 g/kg まで成長
した時点で打ち切り。 出力 storage は 65 step 分の partial dataset として使う。
"""
import time
...
def main():
    ...
    storage_dir = out_dir / "phase1_arabas_cumulus_widened_storage"
    ...
    settings.grid = (100, 100)
    settings.size = (6000 * si.metres, 6000 * si.metres)  # cell 60m 維持、 既存と同解像度、 domain 4 倍
    settings.simulation_time = 5400 * si.second

    n_sd = settings.n_sd_per_gridbox * settings.grid[0] * settings.grid[1]
    print(f"=== Arabas 2015 強上昇流 + 4 倍 domain (b113) (rhod_w_max=2.0) 試走 ===")
