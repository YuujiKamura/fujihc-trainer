...
export const STRAVA_NAME_SUFFIX = ' (fujihill-trainer simulator)';
// description prepend: caller 渡し description の先頭に必ず prepend、 既に冒頭に含まれて
// いれば二重 prepend しない。 改行 2 つで本文と分離。
export const STRAVA_DESCRIPTION_PREFIX = 'This is an indoor trainer simulation of the Mt. Fuji Hill Climb course generated by fujihill-trainer. Not an actual outdoor activity.\n\n';
...
export function withTrademarkSuffix(name) {
  const base = typeof name === 'string' && name.length > 0 ? name : 'fujihill ride';
 ...
 * brief 34 ε-4: name に「(fujihill-trainer simulator)」接尾 + description に商標混同対策 prefix を
...
export async function postUpload(
  ...
  // description prefix も同様。 caller が省略しても fujihill-trainer simulator 表記が必ず付く。
